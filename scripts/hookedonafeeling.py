...
class Geometry:
    ...
    def sin_z(form, t):
        ...
        for i in rotation_order:
            new_form[2, i] = 0.4 * np.sin(t + i*2*np.pi/n_drones)
        return new_form
...
g = Geometry()
#ooga chaka
g.sin_wave(form=formTakeoff, n=4, duration=9)
#IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII gotta feeling
g.goto(form=formCircle, duration=2)
g.spiral(form=formCircle, z=1, n=7, duration=12)
#less popular part of chorus
g.goto(form=formCircle, duration=2)
g.rotate(form=formTriangle, n=9, duration=17)
#before alcohol
g.rotate(form=formCircle, n=8, duration=14)
...
tgen.trajectories_to_json(trajectories, 'scripts/data/HookedOnAFeeling.json')
tgen.plot_trajectories(trajectories)
tgen.animate_trajectories('HookedOnAFeeling.mp4', trajectories, 1)
