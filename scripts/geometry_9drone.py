...
if 'pybswarm/scripts' in os.getcwd():
    os.chdir('..')
...
circle_order = [5, 4, 3, 6, 8, 2, 7, 0, 1]
...
for i, order in enumerate(circle_order):
    if i == 4:
        ...
        theta = (order)*2*np.pi/(n_drones - 1)
        points.append([radius*np.cos(theta), radius*np.sin(theta), 0])

formCone = scale_formation(np.array(points).T, form_scale)
plot_formation(formCone, 'cone')
# %%
points = []
radius = 1

for i, order in enumerate(circle_order):
    theta = order*2*np.pi/(n_drones)
    points.append([radius*np.cos(theta), radius*np.sin(theta), 0])
