...
class QtmWrapper(Thread):
    ...
    async def _connect(self):
        ...
        host = '192.168.1.16'
        self.connection = await qtm.connect(host, version='1.20')
...
def run_waypoints(cf, waypoints):
...
if __name__ == "__main__":
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        if args.mode == 'traj':
            ...
            print('Running waypionts...')
            deactivate_high_level_commander(cf)
            reset_estimator(cf)

            run_waypoints(cf, waypoints)
        else:
            print('Not a valid mode, either "traj" or "waypoint"')
